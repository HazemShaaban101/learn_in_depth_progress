...
// requirements:
//		APB1 clock speed is 16 MHz
//		APB2 clock speed is 8 MHz
//		AHB clock speed is 32 MHz
//		SYSCLK clock speed is 32 MHz
//		only use HSI clock
...
void clock_init()
{
	// PLL is using internal clock / 2, we need to make the PLL multiply it by 8
	RCC_CFGR |= 6 << 18;		// now PLL will output 32 MHz
	
	// HSI is set by default as the default clock, PLL is off so we need to turn it on
	RCC_CR |= 1<<24;
	
	// SW reg is set by default to take HSI as SYSCLK so we need to configure it to take PLL
	RCC_CFGR |= 1 << 1; // now SYSCLK is 32 MHz
