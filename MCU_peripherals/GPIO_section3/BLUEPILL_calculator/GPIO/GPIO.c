		...
		switch (PinMode)
		{
			...
			case GPIO_MODE_IN_PU:
				*config_reg |= 0b1000 << get_CRHL_shift(PinNumber);  		// CNF = 10		MODE = 00
				GPIOx->ODR |= 1UL << PinNumber;								// ODR for the desired pin is set to HIGH to enable internal Pull-up resistance
				break;
			case GPIO_MODE_IN_PD:
				*config_reg |= 0b1000 << get_CRHL_shift(PinNumber);  		// CNF = 10		MODE = 00
				GPIOx->ODR &= ~(1UL << PinNumber);							// ODR for the desired pin is set to LOW to enable internal Pull-down resistance
				break;
			case GPIO_MODE_AFI:
...
{
	return ((GPIOx->IDR & (1UL << PinNumber)) >> PinNumber);
}
...
* @brief 			- Changes operation speed of a specific pin
* @param [in] 		- GPIOx: pointer to the desired GPIO port, the pointer is casted to GPIO_t which is a structure that contains all GPIO registers
