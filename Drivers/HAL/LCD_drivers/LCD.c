/************************************************************************************************
				...
				this file include:
					...
					-set_DataPins_Direction(): since each time we check the busy state of the LCD we have to change pins direction, a function was implemented to do sopen
					-LCD_init(): initializes the pins used by LCD, then sends initialization sequence to LCD
					-LCD_kick(): to signal the LCD to read/write
					-LCD_isbusy(): checks if LCD is busy
					-LCD_send_command(): sends a command using command mode on the LCD
					-LCD_send_character(): sends a character using data mode on the LCD
					-LCD_send_string(): sends an entire string to LCD
					-LCD_send_number(): sends an integer number to LCD
					-LCD_clear_display(): sends the clear screen command to LCD
					-LCD_goto_XY(): sends coordinates for the LCD cursor to go to
					
				this file is incremental, meaning it is constantly updated based on
				my next project requirements. it is not fully functional for all LCD
